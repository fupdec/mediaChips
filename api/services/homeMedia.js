...
async function getContinueWatching(db, limit = 12) {
...
async function getFavoriteMedia(db, limit = 12) {
  const [rows] = await db.sequelize.query(
    `${MEDIA_HOME_SELECT}
     ${MEDIA_HOME_FROM}
...
async function getTopViewedMedia(db, limit = 12) {
  const [rows] = await db.sequelize.query(
    `${MEDIA_HOME_SELECT}
     ${MEDIA_HOME_FROM}
     WHERE media.views > 0
...
async function getHomeMedia(db, limits = {}) {
