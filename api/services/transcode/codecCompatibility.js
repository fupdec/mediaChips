...
function getPrimaryStream(streams, type) {
  return (streams || []).find((stream) => stream.codec_type === type) || null
}

function analyzeProbeResult(probe, filePath, {audioOnly = false} = {}) {
  ...
  if (audioOnly || (!videoStream && audioStream)) {
    ...
    if (audioStream && !BROWSER_AUDIO_CODECS.has(audioStream.codec_name)) {
      return {
        playable: false,
        reason: 'audio_codec',
        videoCodec: null,
  ...
  if (!BROWSER_VIDEO_CODECS.has(videoStream.codec_name)) {
    return {
      playable: false,
      reason: 'video_codec',
      videoCodec: videoStream.codec_name,
