...
async function loadMissingMedia(db, {shouldStop = () => false, onProgress} = {}) {
...
async function getMissingMediaStatus(db) {
  const missing = await loadMissingMedia(db)

  return {
    total: await db.Media.count(),
    missing: missing.length,
    withHash: missing.filter((item) => item.contentHash).length,
    withoutHash: missing.filter((item) => !item.contentHash).length,
  }
}

function buildExtensionRegex(mediaTypes) {
  ...
  for (const mediaType of mediaTypes) {
    String(mediaType.extensions || '')
      ...
      .filter(Boolean)
      .forEach((ext) => extensions.add(ext.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')))
  }
...
async function* walkMediaFiles(rootDirs, {extensionRegex, shouldStop = () => false}) {
...
function buildMissingIndexes(missingMedia) {
...
function pickWeakCandidate(candidates, foundPath) {
...
async function* iterateMissingMediaSearch(db, options = {}) {
  const {
    folders = [],
    shouldStop = () => false,
  } = options
