  ...
  <v-dialog v-model="$store.state.Tags.dialogEditTag" scrollable persistent>
    <v-card>
      <v-card-title class="edit-card-title">
        ...
        />
        <div>
          <div class="font-weight-light body-1">Editing the tag</div>
          <div class="font-weight-bold">{{tag.name}} 
            <v-tooltip bottom>
                    ...
                    <v-col cols="12" align="center" justify="center">
                      ...
                      <div class="editable-text-field">
                        <v-tooltip bottom v-if="isTagNameEditEnabled">
                          <template v-slot:activator="{ on }">
                            <v-icon left @click="isTagNameEditEnabled=!isTagNameEditEnabled" v-on="on">mdi-close</v-icon>
                          </template>
...
export default {
  ...
  methods: {
    ...
    saveTagInfo() {
      ...
      info.id = this.tag.id
      info.info = true
      info.name = this.tagName
      info.category = this.tag.category
