<template>
  <vuescroll ref="mainContainer" @handle-scroll="handleScroll">
    ...
    <div class="headline text-h3 d-flex align-center justify-center my-6">
      <v-icon x-large left>mdi-tag-outline</v-icon> Tags
      <span class="text-h5 ml-2">({{$store.getters.filteredTagsTotal}})</span>
    </div>
  ...
  mounted() {
    this.$nextTick(function () {
      ...
      this.$store.state.Tags.selection = new Selection({
        ...
        const targetEl = event.target.closest('.tag-card')
        if (event.button==0 && targetEl) this.$store.state.Tags.selection.select(targetEl)
