...
export function useWatcher(apiUrl) {
  ...
  // Флаг для отслеживания состояния WebSocket
  ...
  // Основные методы
  const runWatcher = () => {
    // Если настройка watchFolders выключена
    ...
    try {
      ...
      watcherStore.ws.onmessage = (msg) => {
        ...
        try {
          ...
          switch (parsedMsg.type) {
            case 'files':
              watcherStore.files = _.uniqBy(parsedMsg.data, 'folder.id')
              break
            case 'closed':
      ...
      watcherStore.ws.onerror = (error) => {
        console.error('WebSocket error:', error)
        watcherStore.busy = false
        isWsReady.value = false
      }

      watcherStore.ws.onclose = (event) => {
  ...
  const updateWatcher = (foldersToWatch) => {
    ...
    if (!isWsReady.value || !watcherStore.ws || watcherStore.ws.readyState !== WebSocket.OPEN) {
      ...
      // Ждем подключения и отправляем update
      setTimeout(() => {
        if (isWsReady.value && watcherStore.ws && watcherStore.ws.readyState === WebSocket.OPEN) {
          sendMessage({
            type: 'update',
  ...
  watch(() => watcherStore.folders, (val) => {
    ...
    foldersWatchTimeout = setTimeout(() => {
      const watched = val.filter(x => x.watch)
      if (watched.length > 0) {
