...
export const documentationTranslations = {
  en: {
    ...
    'app.first_launch': { name: 'First launch', content: `<p>A good first setup is:</p><ol><li>Open <b>Settings → Custom metadata</b> and create the fields you need.</li><li>Create at least one field of the <b>Tags</b> type if you want tag categories.</li><li>Open <b>Settings → Media types</b> and pin fields to the Video media type.</li><li>Open a media page and add files or folders.</li><li>Use filters, view settings and saved tabs to organize the library.</li></ol>` },
    ...
    playlists: { name: 'Playlists', content: `<p>The Playlists page lists saved playback queues. A playlist card shows thumbnails from its first videos.</p><p>Click the thumbnails to start playback from the first item, or click the playlist name to edit it. Playlists can also be used by the player for previous/next navigation.</p>` },
    ...
    sets: { name: 'Settings', content: `<p>Settings contain general behavior, appearance, tools, custom metadata, media types, database management and registration/about information.</p>` },
    'settings.general': { name: 'General', content: `<p>General settings control language, browser access hints, video player options and login-related options. The local server address shown on the home page can be copied for access from another device.</p>` },
    'settings.appearance': { name: 'Appearance', content: `<p>Appearance settings control dark mode, theme colors, cards and page layout. Page-level appearance can also be changed from the toolbar on media and tag pages.</p>` },
    ...
    'sets.tools': { name: 'Tools', content: `<p>Tools help maintain the library: quick tag creation from file paths, bulk path editing, watched folders, preview generation and optional data scraping.</p>` },
    ...
    database: { name: 'Database', content: `<p>The database settings manage local databases, backups and generated images. A database contains your catalog data; generated previews and images may also be included in backups.</p>` },
  ...
  ru: {
    ...
    'app.first_launch': { name: 'Первый запуск', content: `<p>Хороший порядок первой настройки:</p><ol><li>Откройте <b>Настройки → Пользовательские метаданные</b> и создайте нужные поля.</li><li>Создайте хотя бы одно поле типа <b>Теги</b>, если нужны категории тегов.</li><li>Откройте <b>Настройки → Типы медиа</b> и закрепите поля к типу Видео.</li><li>Откройте страницу медиа и добавьте файлы или папки.</li><li>Используйте фильтры, настройки вида и вкладки для организации библиотеки.</li></ol>` },
    ...
    playlists: { name: 'Плейлисты', content: `<p>Страница плейлистов показывает сохранённые очереди воспроизведения. Карточка плейлиста отображает превью первых видео.</p><p>Нажмите на превью, чтобы начать воспроизведение с первого элемента, или на название, чтобы редактировать плейлист. Плеер использует плейлист для перехода к предыдущему и следующему видео.</p>` },
    ...
    sets: { name: 'Настройки', content: `<p>В настройках находятся общее поведение, внешний вид, инструменты, пользовательские метаданные, типы медиа, управление базой данных и информация о регистрации.</p>` },
    'settings.general': { name: 'Общие', content: `<p>Общие настройки управляют языком, подсказками доступа из браузера, параметрами видеоплеера и входом. Адрес локального сервера на главной странице можно скопировать для доступа с другого устройства.</p>` },
    'settings.appearance': { name: 'Внешний вид', content: `<p>Настройки внешнего вида управляют тёмным режимом, цветами темы, карточками и макетом страниц. Вид конкретной страницы также можно менять с панели на страницах медиа и тегов.</p>` },
    ...
    'sets.tools': { name: 'Инструменты', content: `<p>Инструменты помогают обслуживать библиотеку: быстро создавать теги из путей, массово редактировать пути, следить за папками, генерировать превью и использовать скрапер данных.</p>` },
    ...
    database: { name: 'База данных', content: `<p>Настройки базы данных управляют локальными базами, резервными копиями и сгенерированными изображениями. База содержит данные каталога; превью и изображения тоже могут включаться в копии.</p>` },
  ...
  es: {
    ...
    'app.first_launch': { name: 'Primer inicio', content: `<p>Una buena configuración inicial es:</p><ol><li>Abra <b>Configuración → Metadatos personalizados</b> y cree los campos necesarios.</li><li>Cree al menos un campo de tipo <b>Etiquetas</b> si necesita categorías de etiquetas.</li><li>Abra <b>Configuración → Tipos de medio</b> y fije campos al tipo Video.</li><li>Abra una página de medios y agregue archivos o carpetas.</li><li>Use filtros, vista y pestañas guardadas para organizar la biblioteca.</li></ol>` },
    ...
    playlists: { name: 'Listas de reproducción', content: `<p>La página muestra colas de reproducción guardadas. Una tarjeta de lista muestra miniaturas de sus primeros videos.</p><p>Haga clic en miniaturas para reproducir desde el primer elemento o en el nombre para editar. El reproductor usa la lista para anterior/siguiente.</p>` },
    ...
    sets: { name: 'Configuración', content: `<p>La configuración incluye comportamiento general, apariencia, herramientas, metadatos personalizados, tipos de medio, base de datos e información de registro.</p>` },
    'settings.general': { name: 'General', content: `<p>La configuración general controla idioma, avisos de acceso desde navegador, opciones del reproductor y acceso. La dirección local de la página de inicio se puede copiar para usar otro dispositivo.</p>` },
    'settings.appearance': { name: 'Apariencia', content: `<p>La apariencia controla modo oscuro, colores, tarjetas y diseño de páginas. La apariencia de una página concreta también puede cambiarse desde la barra en páginas de medios y etiquetas.</p>` },
    ...
    'sets.tools': { name: 'Herramientas', content: `<p>Las herramientas mantienen la biblioteca: creación rápida de etiquetas desde rutas, edición masiva de rutas, carpetas vigiladas, generación de vistas previas y extracción de datos opcional.</p>` },
    ...
    database: { name: 'Base de datos', content: `<p>La configuración de base de datos administra bases locales, copias e imágenes generadas. Una base contiene datos del catálogo; las vistas previas también pueden incluirse en copias.</p>` },
  ...
  cn: {
    ...
    'app.first_launch': { name: '首次启动', content: `<p>建议的初始设置流程：</p><ol><li>打开 <b>设置 → 自定义元数据</b> 并创建需要的字段。</li><li>如果需要标签分类，至少创建一个 <b>标签</b> 类型字段。</li><li>打开 <b>设置 → 媒体类型</b>，将字段固定到视频类型。</li><li>打开媒体页面并添加文件或文件夹。</li><li>使用筛选器、视图设置和标签页组织媒体库。</li></ol>` },
    ...
    playlists: { name: '播放列表', content: `<p>播放列表页面显示已保存的播放队列。播放列表卡片会显示前几个视频的缩略图。</p><p>点击缩略图从第一个项目开始播放，点击名称可编辑播放列表。播放器使用播放列表进行上一个/下一个导航。</p>` },
    ...
    sets: { name: '设置', content: `<p>设置包含常规行为、外观、工具、自定义元数据、媒体类型、数据库管理以及注册/关于信息。</p>` },
    'settings.general': { name: '常规', content: `<p>常规设置控制语言、浏览器访问提示、视频播放器选项和登录相关选项。首页显示的本地服务器地址可复制到其他设备访问。</p>` },
    'settings.appearance': { name: '外观', content: `<p>外观设置控制深色模式、主题颜色、卡片和页面布局。媒体和标签页面上的工具栏也可更改单页外观。</p>` },
    'media.types': { name: '媒体类型', content: `<p>媒体类型按扩展名分组文件，并决定哪些元数据字段可用于对应媒体页面。视频是主要支持的类型。</p><p>打开媒体类型可更改固定字段和导航可见性。</p>` },
    'sets.tools': { name: '工具', content: `<p>工具用于维护媒体库：从路径快速创建标签、批量编辑路径、监听文件夹、生成预览以及可选的数据抓取。</p>` },
    ...
    database: { name: '数据库', content: `<p>数据库设置管理本地数据库、备份和生成图片。数据库包含目录数据；生成的预览和图片也可包含在备份中。</p>` },
