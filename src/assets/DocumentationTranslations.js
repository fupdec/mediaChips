...
export const documentationTranslations = {
  en: {
    ...
    'settings.general': { name: 'General', content: `<p>General settings control language, browser access hints and login-related options. The local server address shown on the home page can be copied for access from another device.</p>` },
    ...
    'settings.video': { name: 'Video', content: `<p>Video settings control the built-in player, preview behavior, image generation and cleanup of generated preview assets.</p>` },
    ...
    'sets.tools': { name: 'Tools', content: `<p>Tools help maintain the library: quick tag creation from file paths, bulk path editing, watched folders and optional data scraping.</p>` },
  ...
  ru: {
    ...
    'settings.general': { name: 'Общие', content: `<p>Общие настройки управляют языком, подсказками доступа из браузера и входом. Адрес локального сервера на главной странице можно скопировать для доступа с другого устройства.</p>` },
    ...
    'settings.video': { name: 'Видео', content: `<p>Настройки видео управляют встроенным плеером, поведением превью, генерацией изображений и очисткой сгенерированных превью.</p>` },
    ...
    'sets.tools': { name: 'Инструменты', content: `<p>Инструменты помогают обслуживать библиотеку: быстро создавать теги из путей, массово редактировать пути, следить за папками и использовать скрапер данных.</p>` },
  ...
  es: {
    ...
    'settings.general': { name: 'General', content: `<p>La configuración general controla idioma, avisos de acceso desde navegador y acceso. La dirección local de la página de inicio se puede copiar para usar otro dispositivo.</p>` },
    ...
    'settings.video': { name: 'Video', content: `<p>La configuración de video controla el reproductor integrado, el comportamiento de vistas previas, la generación de imágenes y la limpieza de recursos generados.</p>` },
    ...
    'sets.tools': { name: 'Herramientas', content: `<p>Las herramientas mantienen la biblioteca: creación rápida de etiquetas desde rutas, edición masiva de rutas, carpetas vigiladas y extracción de datos opcional.</p>` },
  ...
  cn: {
    ...
    'settings.general': { name: '常规', content: `<p>常规设置控制语言、浏览器访问提示和登录相关选项。首页显示的本地服务器地址可复制到其他设备访问。</p>` },
    'settings.appearance': { name: '外观', content: `<p>外观设置控制深色模式、主题颜色、卡片和页面布局。媒体和标签页面上的工具栏也可更改单页外观。</p>` },
    'settings.video': { name: '视频', content: `<p>视频设置控制内置播放器、预览行为、图像生成以及生成预览资源的清理。</p>` },
    'media.types': { name: '媒体类型', content: `<p>媒体类型按扩展名分组文件，并决定哪些元数据字段可用于对应媒体页面。视频是主要支持的类型。</p><p>打开媒体类型可更改固定字段和导航可见性。</p>` },
    'sets.tools': { name: '工具', content: `<p>工具用于维护媒体库：从路径快速创建标签、批量编辑路径、监听文件夹以及可选的数据抓取。</p>` },
