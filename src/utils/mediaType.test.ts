...
const sampleMediaTypes: MediaType[] = [
...
describe('mediaType', () => {
  it('normalizes media type keys', () => {
    ...
    expect(getMediaTypeKey({ id: 0, type: 'Audio' })).toBe('audio')
    expect(getMediaTypeKey({ id: 0, name: 'Video' })).toBe('video')
  })

  it('detects media type categories', () => {
    expect(isVideoMediaType({ id: 0, type: 'video' })).toBe(true)
    expect(isAudioMediaType({ id: 0, type: 'audio' })).toBe(true)
    expect(isVideoMediaType({ id: 0, type: 'audio' })).toBe(false)
  })
  ...
  it('maps delete asset folders', () => {
    expect(getMediaDeleteAssetFolder({ id: 0, type: MEDIA_TYPE_VIDEO })).toBe('videos')
    expect(getMediaDeleteAssetFolder({ id: 0, type: MEDIA_TYPE_AUDIO })).toBe('audios')
    expect(getMediaDeleteAssetFolder({ id: 0, type: 'unknown' })).toBeNull()
  })

  it('matches allowed media types', () => {
    const video = { id: 0, type: 'video' }
