...
export function detectWheelDevice(event) {
...
function takeAccumulated(key, delta, threshold) {
...
function applyWheelModifiers(value, event) {
  if (event.shiftKey) return value * 4
  if (event.ctrlKey || event.metaKey) return value / 2
  return value
}

function resolveWheelMagnitude(event, device) {
...
function finalizeWheelValues(values, event, {slider = false} = {}) {
...
export function getSeekSecondsFromWheel(event) {
...
export function getVolumeDeltaFromWheel(event, {slider = false} = {}) {
  const device = detectWheelDevice(event)
  const values = finalizeWheelValues(resolveWheelMagnitude(event, device), event, {slider})
  return values.volumeDelta
}

export function getMarkRangeDeltaFromWheel(event) {
  const device = detectWheelDevice(event)
  const profile = WHEEL_PROFILES[device]
  const primary = getPrimaryWheelDelta(event)
...
export function shouldUseWheelSeek(event) {
  return Boolean(event.altKey || event.shiftKey || event.ctrlKey || event.metaKey)
}

export function preventWheelDefault(event) {
